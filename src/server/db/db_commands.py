...
def add_feature(household_id, feature_name, feature_type, x_pos, y_pos, z_pos, icon='home-outline'):
    ...
    with conn.cursor() as cursor:
        ...
        cursor.execute("""
            INSERT INTO Feature_Encrypted (household_id, feature_name, feature_type, x_pos, y_pos, z_pos, icon)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
            RETURNING feature_id
        """, (household_id, psycopg2.Binary(feature_name_bytes), psycopg2.Binary(feature_type_bytes), x_pos, y_pos, z_pos, icon))
...
def update_household(household_id, household_name):
    ...
    with conn.cursor() as cursor:
        cursor.execute("""
            ...
            WHERE household_id = %s
        """, (psycopg2.Binary(household_name_bytes), household_id,))
    conn.commit()
